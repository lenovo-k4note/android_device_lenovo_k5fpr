...
static void handleNotification(const LightState& state) {
    uint32_t blueBrightness;

    blueBrightness = rgbToBrightness(state);
...
static void handleBattery(const LightState& state) {
    uint32_t redBrightness;
	
    redBrightness = rgbToBrightness(state);

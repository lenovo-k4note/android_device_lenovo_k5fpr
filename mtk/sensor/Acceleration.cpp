...
int AccelerationSensor::FindDataFd() {
    ...
    if (fd >= 0)
    {
        ...
        else
        {
            buf[len] = '\0';
            sscanf(buf, "%d\n", &num);
            ALOGD("len = %d, num = %d, buf = %s",len,num,buf);
        }
    }else{
        return -1;
    }
    sprintf(buf_s, "/dev/input/event%d", num);
    fd = open(buf_s, O_RDONLY | O_NONBLOCK);
    if (fd < 0) {
    	ALOGE("%s: unable to open %s, err %d\n", __func__, buf_s, errno);
    }
    ALOGD("fd = %d", fd);
        ...
        else if (type == EV_SYN)
        {
            ...
            if (mEnabled) {
                 ...
                 if (time >= mEnabledTime) {
					/* ALOGE("AccelerationSensor: time(%lld), timestamp(%lld), mAllInputDataCompleteFlag:%d\n", 
						time, mPendingEvent.timestamp, mAllInputDataCompleteFlag); */
